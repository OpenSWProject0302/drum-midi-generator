...
def play_drum(track: MidiTrack, pattern: dict, start_offset: int = 0):
    ...
    for step in range(16):  # 한 마디 = 16 step (16분음표)
        ...
        time = NOTE_LENGTH["SIXTEENTH"]
        if step == 0:
            time = start_offset if start_offset > 0 else NOTE_LENGTH["SIXTEENTH"]

        if active_notes:
            for i, note in enumerate(active_notes):
                playOn(track, note, time=time if i == 0 else 0, channel=DRUM_CHANNEL)
            for i, note in enumerate(active_notes):
                playOff(track, note, time=NOTE_LENGTH["SIXTEENTH"] if i == 0 else 0, channel=DRUM_CHANNEL)
        else: # 쉼표 구간이면 시간만 흐르게 함
            track.append(Message('note_off', note=35, velocity=0, time=time, channel=DRUM_CHANNEL))
